...
const completionSource = await readFile(
  new URL(
    '../../src/components/ma-professor/assessments/ufcdCompletionDate.ts',
    import.meta.url
  ),
  'utf8'
)
...
test(
  'CFP model preserves configured criteria, ACS and the defensive module-name cleanup',
  () => {
    assert.match(
      modelSource,
      /slice\(0, 6\)/
    ...
    assert.match(
      modelSource,
      /usesAcs[\s\S]*criterionScores/
    )
    assert.match(
      modelSource,
      /acsScore:[\s\S]*usesAcs/
    )
    assert.match(
      modelSource,
      /fichas\?\\s\+de\\s\+avalia/
    )
    assert.match(
      modelSource,
      /processNumber:\s*''/
...
test(
  'official Excel export uses the supplied macro-enabled workbook instead of rebuilding an xlsx',
  () => {
...
test(
  'instrument layouts preserve the real P1 versus P2/P3 column offsets and ACS rows',
  () => {
    assert.match(
      excelSource,
      /P1_LAYOUT[\s\S]*titleCell:\s*'G2'[\s\S]*firstItemColumn:\s*4[\s\S]*lastItemColumn:\s*49/
    )
    assert.match(
      excelSource,
      /P23_LAYOUT[\s\S]*titleCell:\s*'F2'[\s\S]*firstItemColumn:\s*3[\s\S]*lastItemColumn:\s*48/
    )
    assert.match(
      excelSource,
    ...
    assert.match(
      excelSource,
      /usesAcs[\s\S]*adaptationColumn[\s\S]*generalColumn/
    )
    assert.match(
      excelSource,
      /snapshot\.studentRows\.length > 30/
    )
  }
)

test(
  'template repair restores missing P1 sheets and the continuous student map without changing VBA in code',
  () => {
    assert.match(
      templateSource,
      /cloneP1/
    ...
    assert.match(
      templateSource,
      /repairP2P3StudentReferences/
    )
    assert.match(
      templateSource,
      /repairAutoStudentReferences/
    )
    assert.match(
      templateSource,
      /'J', 'L', 'N', 'P', 'R', 'T'/
...
test(
  'module completion date comes from taught periods that count toward progress, not grade confirmation',
  () => {
    assert.match(
      completionSource,
    ...
    assert.match(
      completionSource,
      /completedPeriods \+=\s*lesson\.periodCount/
    )
    assert.match(
      completionSource,
