...
| F103 | Importação automática e segura de critérios de avaliação por PDF | `work/f103-criteria-pdf-import` | `src/components/ma-professor/setup/AssessmentCriteriaPdfImportPanel.tsx` (novo); `src/components/ma-professor/setup/assessmentCriteriaPdfParser.ts` (novo); `src/components/ma-professor/setup/assessmentCriteriaPdfParserCore.ts` (novo); `src/components/ma-professor/setup/SetupWizard.tsx`; `tests/ma-professor/assessment-criteria-pdf-import.test.mjs` (novo) | VALIDADO — PR #50 aberto; análise local + revisão + proteção de critérios existentes implementadas; testes Conquistador/MA-Professor/MA-Quadro e build passaram no Build Check #1707; aguarda autorização explícita para merge | `747b9fcfa5ce0d5dea524a7e2aaf1d0960ddab74` | 2026-09-09 |
