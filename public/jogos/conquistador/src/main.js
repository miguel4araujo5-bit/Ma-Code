...
import {
  OnlineGameClient,
} from './online/OnlineGameClient.js?v=online-presence-2';
...
function createOnlinePlayerView(
  rawPlayer,
) {
  const resources =
    rawPlayer?.resources &&
    typeof rawPlayer.resources ===
      'object'
      ? {
          ...rawPlayer.resources,
        }
...
function getOnlineViewerPlayer() {
  if (!isOnlineGame()) {
    return null;
  }
...
async function startOnlineSession({
  ...
  try {
      ...
      (error) => {
        setStatus(
          error instanceof Error
            ? error.message
            : 'A ligação à partida online foi interrompida.',
          'error',
        );
      },
    );
    ...
    if (!state?.game) {
      throw new Error(
        'A partida online ainda não está disponível.',
      );
    }
    ...
    statusMessage =
      state.actions
        ...
        : `Partida online sincronizada. Aguarde por ${game.currentPlayer?.name || 'outro jogador'}.`;
    ...
    alert(
      error instanceof Error
        ? error.message
        : 'Não foi possível entrar na partida online.',
    );
...
async function runOnlineCommand(
  type,
  payload = {},
) {
  if (
    ...
    return {
      success: false,
      reason:
        'A partida online não está ligada.',
    };
  }
  ...
  if (onlineBusy) {
    return {
      success: false,
      reason:
        'A ação anterior ainda está a ser sincronizada.',
    };
  }
  ...
  try {
    ...
    return {
      ...
      reason:
        error instanceof Error
          ? error.message
          : 'Não foi possível sincronizar a ação.',
    };
  } finally {
    onlineBusy =
      false;
...
function saveGame({
  ...
  if (isOnlineGame()) {
    if (!silent) {
      setStatus(
        'A partida online é guardada automaticamente no servidor.',
        'success',
      );
    }
      ...
      return `
        <div class="instruction-card">
          ...
          <span>
            ...
            O tabuleiro é atualizado automaticamente.
          </span>
        </div>
      `;
    }
...
async function handleVertexClick(
  vertexId,
) {
  let result =
    null;
...
async function handleTerritoryClick(
  territoryId,
) {
  if (
    game.phase !==
      ...
      async () => {
        ...
        statusMessage =
          game.sevenEvent
            ...
            'discard'
            ? isOnlineGame()
              ? 'Descarte confirmado. A aguardar o próximo jogador.'
      ...
      async () => {
        ...
        statusMessage =
          ...
          7
            ...
            : `Resultado ${roll.total}: a produção foi resolvida.`;
