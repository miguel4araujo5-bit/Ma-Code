...
function sortLessons(rows: CalendarLessonRow[]) {
  return [...rows].sort((left, right) => {
    const timeComparison = left.lesson.startTime.localeCompare(
...
function resolveSelectedLessonId(
  ...
  if (
    requestedLessonId &&
    lessons.some(row => row.lesson.id === requestedLessonId)
  ) {
    return requestedLessonId
  }

  if (date === todayISO()) {
    const currentTime = new Intl.DateTimeFormat('pt-PT', {
      hour: '2-digit',
      minute: '2-digit',
      hour12: false
    }).format(new Date())
    ...
    const currentLesson = lessons.find(
      row =>
        ...
        row.lesson.startTime <= currentTime &&
        row.lesson.endTime >= currentTime
    )

    if (currentLesson) {
      return currentLesson.lesson.id
    }
  }

  return lessons[0]?.lesson.id ?? null
}

function buildStudentRows(
  attendance: LessonAttendanceRegister,
  assessmentWorkspace: LessonAssessmentWorkspace,
  assessmentRegister: Awaited<
    ReturnType<typeof assessmentRepository.getAssessmentRegister>
  > | null,
  absenceSummaries: StudentAbsenceSummary[],
  provisionalAverageByStudent: Map<EntityId, Score | null>
): DailyStudentRow[] {
  const assessmentResultByStudent = new Map(
    assessmentRegister?.rows.map(row => [row.student.id, row.result]) ?? []
  )

  const absenceSummaryByStudent = new Map(
    absenceSummaries.map(summary => [summary.studentId, summary])
  )

  const students =
    attendance.rows.length > 0
      ? attendance.rows.map(row => row.student)
      : assessmentWorkspace.students

  const attendanceByStudent = new Map(
    attendance.rows.map(row => [row.student.id, row])
  )

  return students.map(student => {
    const attendanceRow = attendanceByStudent.get(student.id)
    const assessmentResult = assessmentResultByStudent.get(student.id)

    return {
      student,
      attendanceStatus:
        attendanceRow?.effectiveStatus ?? 'present',
      attendanceCode:
        attendanceRow?.effectiveCode ?? '',
      attendanceNote:
        attendanceRow?.effectiveNote ?? '',
      assessmentStatus:
        assessmentResult?.status ?? 'not_evaluated',
      assessmentScore:
        assessmentResult?.status === 'evaluated'
          ? assessmentResult.score
          : null,
      assessmentNote:
        assessmentResult?.note ?? '',
      provisionalAverage:
        provisionalAverageByStudent.get(student.id) ?? null,
      absenceSummary:
        absenceSummaryByStudent.get(student.id) ?? null
    }
  })
}

function buildAssessmentEntries(
  rows: DailyStudentSaveDraft[]
): AssessmentResultDraft[] {
  return rows.flatMap<AssessmentResultDraft>(row => {
    if (row.assessmentStatus === 'not_evaluated') {
      return []
    }

    if (row.assessmentStatus === 'evaluated') {
      if (
        row.assessmentScore === null ||
        !Number.isFinite(row.assessmentScore) ||
        row.assessmentScore < 0 ||
        row.assessmentScore > 20
      ) {
        throw new Error(
...
export class DailyWorkspaceRepository {
  async getDateWorkspace(
    ...
    const calendar = await calendarWorkspaceRepository.getWorkspace(
      academicYearId,
      'week',
      date,
      {}
  ...
  async getLessonWorkspace(
    ...
    const context =
      await calendarWorkspaceRepository.getLessonEditorContext(lessonId)
    ...
    const [
      ...
      attendanceRepository.getLessonAttendanceRegister(lessonId),
      assessmentRepository.getLessonAssessmentWorkspace(lessonId),
      attendanceRepository.listModuleAbsenceSummaries(
        context.lessonRow.module.id
      ),
      assessmentWorkspaceRepository.getWorkspace(academicYearId, {
        teachingAssignmentId: context.lessonRow.assignment.id,
        moduleId: context.lessonRow.module.id
      })
    ])
    ...
    const selectedAssessmentItem =
      requestedAssessmentId === null
        ? null
        : assessmentWorkspace.assessments.find(
            item => item.assessment.id === requestedAssessmentId
          ) ?? assessmentWorkspace.assessments[0] ?? null

    const assessmentRegister = selectedAssessmentItem
      ? await assessmentRepository.getAssessmentRegister(
          selectedAssessmentItem.assessment.id
        )
      : null

    const provisionalAverageByStudent = new Map(
      assessmentOverview.studentRows.map(row => [
        row.student.id,
    ...
    return {
      ...
      assessmentWorkspace,
      selectedAssessment:
        selectedAssessmentItem?.assessment ?? null,
      selectedCriterion:
        selectedAssessmentItem?.criterion ?? null,
  ...
  async saveLesson(
    ...
    if (input.status === 'taught' && !input.summary.trim()) {
      throw new Error(
    ...
    let assessmentEntries: AssessmentResultDraft[] | null = null
    let assessmentTitle = ''

    if (input.assessment.mode !== 'none') {
      if (input.status !== 'taught') {
        throw new Error(
          'A avaliação só pode ser guardada numa aula marcada como dada.'
        )
      }

      assessmentTitle = input.assessment.title.trim()

      if (!assessmentTitle) {
        throw new Error('Indique o nome da atividade de avaliação.')
      }

      if (!input.assessment.criterionId) {
        throw new Error('Selecione o critério de avaliação.')
      }

      assessmentEntries = buildAssessmentEntries(input.students)

      if (input.assessment.mode === 'existing') {
        if (!input.assessment.assessmentId) {
          throw new Error(
        ...
        if (
          !assessmentWorkspace.assessments.some(
            item =>
              item.assessment.id === input.assessment.assessmentId
          )
        ) {
          throw new Error(
    ...
    if (input.status !== 'taught') {
      const [attendance, assessments] = await Promise.all([
        attendanceRepository.getLessonAttendanceRegister(input.lessonId),
        assessmentRepository.getLessonAssessmentWorkspace(input.lessonId)
      ])

      const hasAttendance = attendance.rows.some(row => row.attendance)
    ...
    let updated = await lessonRepository.updateLesson(input.lessonId, {
      status: input.status,
    ...
    if (
      input.giaeStatus === 'submitted' &&
      updated.status === 'taught' &&
      updated.summary.trim()
    ) {
      updated = await lessonRepository.markGIAESubmitted(updated.id)
    } else if (updated.giaeStatus === 'submitted') {
      updated = await lessonRepository.markGIAEPending(updated.id)
    }
    ...
    const attendanceEntries: AttendanceEntryDraft[] = input.students.map(
      row => ({
        studentId: row.studentId,
    ...
    if (input.assessment.mode === 'new') {
      const assessment = await assessmentRepository.createLessonAssessment({
        lessonId: updated.id,
        criterionId: input.assessment.criterionId,
        title: assessmentTitle,
        activityType: input.assessment.activityType,
