...
export default function DailyWorkspaceView({
  ...
  const weekTimeSlots = useMemo(() => {
    ...
    workspace?.weekDays.forEach(day => {
      day.lessons.forEach(row => {
        ...
        if (!current || row.lesson.endTime > current.endTime) {
  ...
  return (
    <main className="min-h-[calc(100vh-58px)] bg-slate-950 px-3 py-3 text-white sm:px-5 lg:px-7">
      <div className="mx-auto max-w-7xl space-y-3">
        <section className="rounded-2xl border border-white/10 bg-slate-900/70 p-3 shadow-xl shadow-black/15">
          <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex min-w-0 items-center gap-2">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-[0.6rem] font-black uppercase tracking-[0.16em] text-cyan-300">
                  Semana
                </p>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button
                type="button"
                onClick={() =>
                  setShowWeekOverview(current => !current)
                }
          ...
          {showWeekOverview ? (
            <div className="mt-3 max-h-72 overflow-auto rounded-xl border border-white/10 bg-slate-950/40">
              <div className="min-w-[760px]">
                <div className="sticky top-0 z-10 grid grid-cols-[5.5rem_repeat(5,minmax(0,1fr))] border-b border-white/10 bg-slate-900/95 backdrop-blur">
                  <div className="flex items-center justify-center border-r border-white/10 px-2 py-2 text-[0.58rem] font-black uppercase tracking-[0.1em] text-slate-500">
                    Hora
                  </div>
                  ...
                  {workspace?.weekDays.map(day => {
                    ...
                    return (
                      <button
                        ...
                        disabled={loading || saving}
                        className={`border-r border-white/10 px-2 py-2 text-center text-[0.66rem] font-black capitalize transition last:border-r-0 ${
                          selected
                ...
                {weekTimeSlots.map(slot => (
                  <div
                    key={slot.startTime}
                    className="grid grid-cols-[5.5rem_repeat(5,minmax(0,1fr))] border-b border-white/10 last:border-b-0"
                  >
                    <div className="flex items-center justify-center border-r border-white/10 bg-slate-900/35 px-1 py-1.5 text-center text-[0.62rem] font-black leading-tight text-slate-300">
                      {slot.startTime}
                    ...
                    {workspace?.weekDays.map(day => {
                      ...
                      return (
                        <div
                          ...
                          className={`min-h-11 border-r border-white/10 p-1 last:border-r-0 ${
                            day.date === date
                              ? 'bg-cyan-300/[0.025]'
                              : ''
                          }`}
                            ...
                            {lessons.map(row => {
                              ...
                              return (
                                <button
                                  ...
                                  className={`w-full rounded-md border px-1.5 py-1 text-left leading-tight transition disabled:opacity-50 ${
                                    active
                                ...
                                >
                                  <span
                                    className={`block truncate text-[0.64rem] font-black ${
                                      active
                                        ? 'text-cyan-100'
                                        : cancelled
                                          ? 'text-rose-100'
                                  ...
                                  <span className="mt-0.5 block truncate text-[0.58rem] font-semibold text-slate-500">
                                    {row.module.code ||
                                      row.module.name}
                                  </span>
                                </button>
          ...
          <div className="mt-2 flex flex-col gap-1 text-[0.68rem] sm:flex-row sm:items-center sm:justify-between">
            <p className="text-slate-500">
              A semana é apenas um apontamento. Selecione uma
              aula para trabalhar abaixo.
            </p>
        ...
        selectedLesson ? (
          <article className="overflow-hidden rounded-2xl border border-white/10 bg-slate-900/60 shadow-xl shadow-black/15">
            <header className="border-b border-white/10 bg-slate-900 px-4 py-3 sm:px-5">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span
                      className={`rounded-full border px-2 py-0.5 text-[0.6rem] font-black uppercase tracking-[0.1em] ${lessonStatusClasses(
                        lessonForm.status
                      )}`}
                    >
            ...
            <div className="space-y-3 p-4 sm:p-5">
              {lessonForm.status === 'cancelled' ? (
                <div className="rounded-xl border border-rose-300/20 bg-rose-300/10 px-4 py-3 text-sm font-bold text-rose-100">
                  Esta aula está cancelada. Pode alterar o
                  estado em “Mais opções”.
                </div>
              ) : null}
              ...
              <section className="rounded-xl border border-white/10 bg-slate-950/55 p-4">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    <p className="text-[0.62rem] font-black uppercase tracking-[0.16em] text-cyan-300">
                      Sumário
                    </p>
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button
                      ...
                      className="rounded-lg border border-cyan-300/20 bg-cyan-300/10 px-3 py-1.5 text-xs font-black text-cyan-100 transition hover:border-cyan-300/40 disabled:cursor-not-allowed disabled:opacity-35"
                    >
                    ...
                    <button
                      ...
                      className="rounded-lg border border-white/10 bg-white/[0.04] px-3 py-1.5 text-xs font-black text-slate-200 transition hover:border-white/20 disabled:cursor-not-allowed disabled:opacity-35"
                    >
                      Copiar anterior
                    </button>
                    ...
                    <button
                      ...
                      className="rounded-lg bg-cyan-300 px-4 py-1.5 text-xs font-black text-slate-950 transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-40"
                    >
                      {saving
                        ? 'A guardar…'
                        : hasUnsavedChanges
                ...
                <textarea
                  ...
                  className={`${inputClassName} mt-3 resize-y text-base leading-7`}
                />

                <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <p className="text-xs leading-5 text-slate-500">
                    Ao escrever um sumário, a aula passa a dada
                    quando guardar.
                  </p>

                  <label className="flex cursor-pointer items-center gap-2 rounded-lg border border-white/10 bg-white/[0.03] px-3 py-2 text-xs font-bold text-slate-300">
                    <input
                      type="checkbox"
                      checked={
              ...
              {showAdvanced ? (
                <section className="rounded-xl border border-amber-300/20 bg-amber-300/[0.05] p-4">
                  ...
                  <label className="mt-4 flex cursor-pointer items-center gap-2 text-sm font-bold text-slate-300">
                    ...
                    Contabilizar estes tempos no progresso da
                    UFCD
                  </label>
              ...
              <section className="overflow-hidden rounded-xl border border-white/10 bg-slate-950/55">
                <div className="flex flex-col gap-3 border-b border-white/10 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-slate-500">
                      {presentCount} presentes · {absentCount}{' '}
                      faltas
                    </p>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button
                      ...
                      className="rounded-lg border border-white/10 bg-white/[0.04] px-3 py-1.5 text-xs font-black text-slate-200 transition hover:border-white/20 disabled:opacity-35"
                    >
                      Todos presentes
                    </button>
                    ...
                    {!assessmentEnabled ? (
                      <button
                        ...
                        className="rounded-lg bg-cyan-300 px-3 py-1.5 text-xs font-black text-slate-950 transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-35"
                      >
                        + Registar avaliação
                      </button>
                    ) : (
                      <button
                        ...
                        className="rounded-lg border border-cyan-300/30 bg-cyan-300/10 px-3 py-1.5 text-xs font-black text-cyan-100"
                      >
                ...
                {!assessmentWorkspace?.criteria.length ? (
                  <div className="border-b border-amber-300/20 bg-amber-300/10 px-4 py-3 text-sm font-bold text-amber-100">
                    Ainda não existem critérios de avaliação
                    para esta disciplina ou UFCD. Pode criá-los
                    no Menu.
                  </div>
                ) : null}

                {assessmentEnabled && assessmentForm ? (
                  <div className="border-b border-cyan-300/15 bg-cyan-300/[0.04] px-4 py-4">
                    <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(14rem,0.7fr)_auto] lg:items-end">
                      ...
                      <button
                        ...
                        disabled={saving}
                        className="rounded-lg border border-white/10 bg-white/[0.04] px-3 py-2.5 text-xs font-black text-slate-300 transition hover:border-rose-300/30 hover:text-rose-100"
                      >
                        {assessmentForm.choice === 'new'
                          ? 'Cancelar avaliação'
                          : 'Fechar avaliação'}
                    ...
                    {showAssessmentDetails ? (
                      <div className="mt-4 grid gap-3 rounded-xl border border-white/10 bg-slate-950/60 p-4 lg:grid-cols-3">
                        <label className="text-xs font-bold text-slate-400">
                ...
                <div
                  className={`grid items-center gap-2 border-b border-white/10 bg-slate-900/80 px-3 py-2 text-[0.62rem] font-black uppercase tracking-[0.1em] text-slate-500 sm:px-4 ${
                    assessmentEnabled
                      ? 'grid-cols-[2.75rem_minmax(0,1fr)_4.75rem_5rem]'
                      : 'grid-cols-[2.75rem_minmax(0,1fr)_4.75rem]'
                  }`}
                >
                  <span className="text-center">N.º</span>
                  <span>Aluno</span>
                ...
                <div className="divide-y divide-white/10">
                  {students.map((row, index) => {
                    const absencePercent =
                      row.absenceSummary?.absencePercent ??
                      null
                    ...
                    return (
                      <div key={row.student.id}>
                        <div
                          className={`grid items-center gap-2 px-3 py-2.5 sm:px-4 ${
                            assessmentEnabled
                              ? 'grid-cols-[2.75rem_minmax(0,1fr)_4.75rem_5rem]'
                              : 'grid-cols-[2.75rem_minmax(0,1fr)_4.75rem]'
                          }`}
                        >
                          ...
                          <div className="min-w-0">
                            ...
                            {(hasAbsenceWarning ||
                              row.provisionalAverage !==
                                null) && (
                              <p className="mt-0.5 truncate text-[0.66rem] font-semibold text-slate-500">
                          ...
                          <button
                            ...
                            className={`rounded-lg border px-2 py-1.5 text-xs font-black transition disabled:opacity-35 ${
                              row.attendanceStatus ===
                              'absent'
                          ...
                          {assessmentEnabled ? (
                            row.assessmentStatus ===
                            'absent' ? (
                              <span className="rounded-lg border border-rose-300/25 bg-rose-300/10 px-2 py-1.5 text-center text-xs font-black text-rose-100">
                                F
                              </span>
                            ) : row.assessmentStatus ===
                              'exempt' ? (
                              <span className="rounded-lg border border-violet-300/25 bg-violet-300/10 px-2 py-1.5 text-center text-xs font-black text-violet-100">
                                D
                              </span>
                            ) : (
                              <input
                ...
                <div className="flex flex-col gap-2 border-t border-white/10 bg-slate-900/50 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                  <p className="text-xs text-slate-500">
                    As opções menos usadas ficam escondidas
                    para manter a vista simples.
                  </p>
                  ...
                  <button
                    ...
                    className="self-start rounded-lg border border-white/10 bg-white/[0.04] px-3 py-1.5 text-xs font-black text-slate-300 transition hover:border-cyan-300/30 hover:text-white sm:self-auto"
                  >
                    {showStudentDetails
            ...
            <footer className="sticky bottom-0 z-20 flex flex-col gap-2 border-t border-white/10 bg-slate-900/95 px-4 py-3 backdrop-blur-xl sm:flex-row sm:items-center sm:justify-between sm:px-5">
              ...
              <button
                ...
                className="rounded-lg bg-cyan-300 px-5 py-2.5 text-sm font-black text-slate-950 transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-40"
              >
                {saving
                  ? 'A guardar…'
                  : hasUnsavedChanges
