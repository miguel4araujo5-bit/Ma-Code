...
const setupSteps: SetupStepDefinition[] = [
  {
    ...
    title: 'Turmas',
    shortTitle: 'Turmas',
    description: 'Selecione rapidamente as turmas do ensino profissional que leciona.'
  },
  {
    id: 'subjects',
    number: 3,
    ...
    description: 'Escolha as disciplinas e associe cada uma às turmas onde a leciona.'
  },
  {
    id: 'modules',
    number: 4,
    ...
    shortTitle: 'UFCD',
    description: 'Introduza cada UFCD uma vez e indique apenas as turmas onde se aplica.'
  },
  {
    id: 'assessment_criteria',
    number: 5,
    title: 'Critérios de avaliação',
    shortTitle: 'Critérios',
    description: 'Configure critérios e ponderações que totalizem 100%.'
  },
  {
    id: 'planifications',
    number: 6,
    title: 'Planificações',
    shortTitle: 'Planos',
    description: 'Organize conteúdos, atividades, objetivos e sumários.'
  },
  {
    id: 'weekly_schedule',
    number: 7,
    ...
    shortTitle: 'Horário',
    description: 'Indique os dias, as horas e os tempos letivos.'
  },
  {
    id: 'students',
    number: 8,
    title: 'Alunos',
    shortTitle: 'Alunos',
    description: 'Adicione o número e o nome dos alunos de cada turma.'
  },
  {
    id: 'confirmation',
    number: 9,
    ...
    description: 'Reveja os dados e conclua a configuração inicial.'
  }
]
...
function normalizeProgressStep(
  step: SetupStepId | null | undefined
): SetupStepId {
  return step === 'academic_year' || !step
    ? 'groups'
    : step
}
    ...
    <div className="grid gap-6 xl:grid-cols-[1fr_0.85fr]">
      <section className="rounded-[1.75rem] border border-white/10 bg-slate-950/70 p-5 shadow-xl shadow-black/20 sm:p-6">
        ...
        <p className="mt-3 text-sm leading-7 text-slate-400">
          O MA-Professor mantém os dados separados por ano letivo, mas não precisa de preencher esta informação durante a configuração inicial.
        </p>
        ...
        <div className="mt-7 rounded-2xl border border-cyan-300/15 bg-cyan-300/[0.055] p-5">
          ...
          <p className="mt-3 text-sm leading-7 text-slate-300">
            {formatDate(snapshot.academicYear.startDate)} a{' '}
            {formatDate(snapshot.academicYear.endDate)}
          </p>
        </div>
      </section>
      ...
      <section className="rounded-[1.75rem] border border-white/10 bg-slate-950/55 p-5 shadow-xl shadow-black/15 sm:p-6">
        ...
        <div className="mt-4 rounded-2xl border border-emerald-300/20 bg-emerald-300/[0.065] p-4">
          <p className="font-black text-white">
            Não precisa de alterar nada aqui.
          </p>

          <p className="mt-1 text-sm leading-6 text-slate-400">
            Mais tarde poderá alternar entre anos letivos existentes no menu principal, sem misturar os respetivos dados.
          </p>
        </div>
        ...
        <button
          type="button"
          onClick={onContinue}
          className="mt-6 inline-flex w-full items-center justify-center rounded-2xl border border-white/10 bg-white/[0.055] px-5 py-3.5 text-sm font-black text-white transition hover:border-cyan-300/25 hover:bg-cyan-300/[0.09]"
        >
...
function EducationModeSelector({
  ...
  const modes: Array<{
  ...
  }> = [
    {
      ...
      description: 'Turmas do 10.º, 11.º e 12.º ano, com UFCD ou módulos.',
      available: true
    },
    {
      id: 'regular',
      title: 'Ensino regular',
      description: 'Configuração para turmas do ensino regular.',
      available: false
    },
    {
      id: 'mixed',
      title: 'Misto',
      description: 'Para docentes que lecionam simultaneamente ensino regular e profissional.',
      available: false
    }
  ]
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <section className="rounded-[2rem] border border-cyan-300/15 bg-slate-950/75 p-6 shadow-2xl shadow-cyan-950/20 backdrop-blur-xl sm:p-8">
        <p className="text-xs font-bold uppercase tracking-[0.18em] text-cyan-200">
          Passo 1 de 9 · Primeira configuração
        </p>

        <h1 className="mt-3 text-3xl font-black tracking-tight text-white sm:text-4xl">
          Que tipo de ensino leciona?
        </h1>

        <p className="mt-4 max-w-3xl text-sm leading-7 text-slate-400 sm:text-base">
          Esta escolha permite ao MA-Professor mostrar apenas o que faz sentido para o seu trabalho e evitar perguntas desnecessárias.
        </p>
          ...
          {modes.map(mode => (
            <button
              ...
              onClick={() => {
                ...
                setNotice(
                  `${mode.title} será disponibilizado numa fase posterior. Para já, estamos a concluir o fluxo do ensino profissional.`
                )
              }}
              className={`rounded-3xl border p-5 text-left transition ${
                mode.available
                  ? 'border-cyan-300/25 bg-cyan-300/[0.07] hover:-translate-y-0.5 hover:border-cyan-300/45 hover:bg-cyan-300/[0.11]'
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                >
                  {mode.available ? 'Disponível' : 'Em breve'}
                </span>
              </div>
        ...
        {notice ? (
          <p className="mt-5 rounded-2xl border border-violet-300/20 bg-violet-300/[0.07] p-4 text-sm leading-6 text-violet-100">
            {notice}
          </p>
        ) : null}
      </section>
...
export default function SetupWizard({
  ...
  const [activeStep, setActiveStep] = useState<SetupStepId>(() =>
    getInitialStep(snapshot)
  )

  const [educationMode, setEducationMode] = useState<'professional' | null>(
    snapshot.groups.length > 0 ? 'professional' : null
  )
  ...
  const currentProgressStep = normalizeProgressStep(
  ...
  const completedSetupSteps = setupSteps.filter(step =>
  ...
  const completionPercent = Math.round(
    (completedCount / totalVisibleSteps) * 100
  )
  ...
  function isStepUnlocked(stepId: SetupStepId) {
    ...
    return (
      ...
      completedSteps.has(previousStep.id) ||
      currentProgressStep === stepId
    )
  }
  ...
  function handleStepCompleted(nextSnapshot: SetupSnapshot) {
    ...
    if (
      nextSnapshot.academicYear.setupCompletedAt ||
      nextSnapshot.progress?.completedAt
    ) {
    ...
    setActiveStep(
      normalizeProgressStep(nextSnapshot.progress?.currentStep)
    )
  ...
  function renderActiveStep() {
    switch (activeStep) {
      ...
      case 'modules':
        return <ModulesSetupStep {...commonProps} />

      case 'assessment_criteria':
        return <AssessmentCriteriaSetupStep {...commonProps} />

      case 'planifications':
        return <PlanificationsSetupStep {...commonProps} />

      case 'weekly_schedule':
        return <WeeklyScheduleSetupStep {...commonProps} />
  ...
  if (!educationMode && snapshot.groups.length === 0) {
    return (
      <EducationModeSelector
        onSelectProfessional={() => setEducationMode('professional')}
      />
    )
  }
    ...
    <div className="mx-auto max-w-[100rem]">
      <section className="rounded-[2rem] border border-cyan-300/15 bg-slate-950/75 p-5 shadow-2xl shadow-cyan-950/20 backdrop-blur-xl sm:p-6 lg:p-7">
        <div className="flex flex-wrap items-start justify-between gap-5">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-cyan-200">
              Configuração · Ensino profissional
            </p>

            <h1 className="mt-3 text-2xl font-black tracking-tight text-white sm:text-3xl">
              Vamos preparar apenas o essencial.
            </h1>

            <p className="mt-2 text-sm leading-6 text-slate-400">
              Ano letivo ativo: {snapshot.academicYear.name}
            </p>
          </div>
          ...
          <div className="min-w-[12rem] rounded-2xl border border-white/10 bg-white/[0.035] p-4">
            <div className="flex items-center justify-between gap-4">
              ...
              <span className="text-sm font-black text-cyan-100">
                {completedCount}/{totalVisibleSteps}
              </span>
            </div>
          ...
          <label className="block">
            ...
            <select
              ...
              onChange={(event: ChangeEvent<HTMLSelectElement>) =>
                navigateToStep(event.target.value as SetupStepId)
              }
            ...
            >
              {setupSteps.map(step => (
                ...
                >
                  ...
                  {completedSteps.has(step.id) ? ' — concluído' : ''}
                </option>
              ))}
            </select>
          </label>
            ...
            return (
              <button
                ...
                className={`min-w-0 rounded-2xl border p-3 text-left transition ${
                  active
                    ? 'border-cyan-300/40 bg-cyan-300/[0.09] shadow-lg shadow-cyan-950/15'
                    : completed
                      ? 'border-emerald-300/20 bg-emerald-300/[0.05] hover:border-emerald-300/35'
                      : unlocked
          ...
          <div className="mt-5 flex flex-wrap items-start justify-between gap-4 rounded-2xl border border-white/10 bg-white/[0.025] p-4">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.14em] text-slate-500">
                Passo {activeStepDefinition.number} de {totalVisibleSteps}
              </p>
            ...
            isStepUnlocked(currentProgressStep) ? (
              <button
                type="button"
                onClick={() => navigateToStep(currentProgressStep)}
                className="shrink-0 rounded-xl border border-violet-300/20 bg-violet-300/[0.07] px-3 py-2 text-xs font-bold text-violet-100 transition hover:bg-violet-300/[0.12]"
              >
      ...
      <p className="mt-6 text-center text-xs leading-6 text-slate-500">
        Os passos concluídos podem ser revistos sem eliminar os dados dos passos seguintes.
      </p>
    </div>
  )
}
