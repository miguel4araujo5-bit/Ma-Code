...
const AccessContext = createContext<AccessContextValue | null>(
  null
)
...
export function AccessGate({
  ...
  const verifyStoredSession = useCallback(async () => {
    ...
    try {
      const response = await verifyMAProfessorAccess(
        stored.token,
        stored.deviceId
      )
  ...
  const requestRenewal = useCallback(
    async (plan: RenewableLicensePlan) => {
      if (!session) {
        throw new Error('A sessão já não está disponível.')
      }

      const response = await requestMAProfessorRenewal(
        session.token,
        session.deviceId,
        plan
      )
  ...
  const handleStart = async (event: FormEvent) => {
    ...
    try {
      ...
      const response = await startMAProfessorBeta(
        normalizedEmail,
        deviceId
      )
  ...
  const handleRenewFromGate = async (
    plan: RenewableLicensePlan
  ) => {
    setError('')
    ...
    try {
      ...
      setError(getErrorMessage(renewalError))
    } finally {
      setRenewingPlan(null)
    }
  }

  const contextValue = useMemo<AccessContextValue | null>(
    () =>
      session
        ? {
            session,
        ...
        : null,
    [refresh, refreshing, requestRenewal, session, signOut]
  )
        ...
        <section className="mx-auto grid max-w-5xl overflow-hidden rounded-[2rem] border border-white/10 bg-slate-900/80 shadow-2xl shadow-cyan-950/30 lg:grid-cols-[1.05fr_0.95fr]">
          <div className="border-b border-white/10 p-7 sm:p-10 lg:border-b-0 lg:border-r">
            ...
            <h1 className="mt-4 text-3xl font-black tracking-tight sm:text-4xl">
              O seu ano letivo num único lugar.
            </h1>
            <p className="mt-4 max-w-xl text-sm leading-7 text-slate-300 sm:text-base">
              Sumários, aulas, turmas, avaliações, faltas,
              planificações e horários, com os dados guardados neste
              dispositivo e cópias de segurança controladas por si.
            </p>
          ...
          >
            ...
            <p className="mt-2 text-sm leading-6 text-slate-400">
              Use o email que ficará associado à sua licença.
              Não é necessário criar uma palavra-passe nesta fase beta.
            </p>
            ...
            <p className="mt-4 text-xs leading-5 text-slate-500">
              Os dados escolares não são enviados para o servidor de
              licenças. Permanecem na base de dados local do browser.
            </p>
          </form>
        </section>
      </main>
  ...
  if (!isLicenseUsable(session.license)) {
    return (
      <main className="flex min-h-screen items-center justify-center bg-slate-950 px-4 py-10 text-white sm:px-6">
        <section className="w-full max-w-2xl rounded-[2rem] border border-amber-300/20 bg-slate-900 p-7 shadow-2xl sm:p-10">
          <p className="text-xs font-black uppercase tracking-[0.2em] text-amber-300">
            Licença {getLicenseStatusLabel(session.license.status)}
          </p>
          <h1 className="mt-3 text-3xl font-black">
            O período de acesso terminou.
          </h1>
          <p className="mt-3 text-sm leading-7 text-slate-300">
            O plano {getLicensePlanLabel(session.license.plan)} terminou em{' '}
          ...
          <div className="mt-7 grid gap-3 sm:grid-cols-2">
            <button
              ...
              onClick={() =>
                void handleRenewFromGate('paid_30_days')
              }
            ...
            <button
              ...
              disabled={Boolean(renewingPlan)}
              onClick={() =>
                void handleRenewFromGate('school_year')
              }
