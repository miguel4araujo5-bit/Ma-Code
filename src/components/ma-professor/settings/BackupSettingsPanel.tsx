...
export function BackupSettingsPanel({
  ...
  const handleJsonExport = () =>
    run(
      ...
      async () => {
        ...
        downloadTextFile(
          getBackupFileName(backup.exportedAt),
  ...
  const handleCsvExport = (
    kind: 'students' | 'lessons' | 'attendance' | 'grades'
  ) =>
    run(
      `csv-${kind}`,
      async () => {
        ...
        const exports = {
          students: {
            name: `ma-professor-alunos-${today()}.csv`,
            content: exportStudentsCsv(backup.data)
          },
          lessons: {
            name: `ma-professor-sumarios-${today()}.csv`,
            content: exportLessonsCsv(backup.data)
          },
          attendance: {
            name: `ma-professor-faltas-${today()}.csv`,
            content: exportAttendanceCsv(backup.data)
          },
          grades: {
            name: `ma-professor-avaliacoes-${today()}.csv`,
            content: exportGradesCsv(backup.data)
          }
        }
  ...
  const handleFile = async (
    ...
    try {
      const parsed = await parseMAProfessorBackupFile(file)
  ...
  const handleRestore = () => {
    if (!pendingBackup || !validation?.valid) {
      return
    }
    ...
    if (restoreConfirmation.trim().toUpperCase() !== 'RESTAURAR') {
      setFeedback({
        tone: 'error',
        message: 'Escreva RESTAURAR para confirmar.'
      })
      return
    }
    ...
    void run(
      'restore',
      async () => {
        await restoreMAProfessorBackup(pendingBackup)
        setPendingBackup(null)
        setValidation(null)
        setRestoreConfirmation('')
        onDataChanged?.()
  ...
  const handleReset = () => {
    if (resetConfirmation.trim().toUpperCase() !== 'APAGAR') {
      setFeedback({
        tone: 'error',
        message: 'Escreva APAGAR para confirmar.'
      })
      return
    }
    ...
    <div className="space-y-6">
      <section className="rounded-3xl border border-cyan-300/15 bg-slate-900/70 p-5 sm:p-6">
        ...
        <h2 className="mt-2 text-xl font-black text-white">
          Proteja todo o seu trabalho
        </h2>
        <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">
          O ficheiro JSON inclui anos letivos, turmas, alunos,
          planificações, aulas, sumários, faltas, avaliações e
          definições. Guarde-o num local seguro e crie uma nova cópia
          regularmente.
        </p>
      ...
      <section className="rounded-3xl border border-white/10 bg-slate-900/70 p-5 sm:p-6">
        ...
        <p className="mt-2 text-sm leading-6 text-slate-400">
          Os CSV usam ponto e vírgula e são compatíveis com o Excel em
          português.
        </p>
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {(
            ...
            >
              {busy === `csv-${kind}` ? 'A exportar…' : label}
            </button>
          ))}
        </div>
      </section>
      ...
      <section className="rounded-3xl border border-white/10 bg-slate-900/70 p-5 sm:p-6">
        ...
        <h2 className="mt-2 text-xl font-black text-white">
          Recuperar uma cópia anterior
        </h2>
        <p className="mt-2 text-sm leading-6 text-slate-400">
          O ficheiro é validado antes de qualquer alteração. O restauro
          substitui todos os dados escolares atualmente guardados neste
          browser.
        </p>
          ...
          <div className="mt-5 rounded-2xl border border-white/10 bg-slate-950/70 p-4">
            <div className="grid gap-3 text-center sm:grid-cols-4">
              <div>
                ...
                <p className="text-xs text-slate-500">Anos letivos</p>
              </div>
              <div>
                <p className="text-2xl font-black text-white">
            ...
            {validation.valid ? (
              <div className="mt-5 grid gap-3 sm:grid-cols-[1fr_auto]">
                ...
                >
                  {busy === 'restore' ? 'A restaurar…' : 'Restaurar'}
                </button>
              </div>
            ) : null}
          </div>
      ...
      <section className="rounded-3xl border border-rose-400/20 bg-rose-400/5 p-5 sm:p-6">
        ...
        <p className="mt-2 text-sm leading-6 text-slate-400">
          Esta operação não pode ser anulada. Crie primeiro uma cópia de
          segurança completa.
        </p>
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-[1fr_auto]">
          ...
          >
            {busy === 'reset' ? 'A apagar…' : 'Apagar tudo'}
          </button>
        </div>
      </section>
