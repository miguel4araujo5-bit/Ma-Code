...
function toPositionedItem(value: unknown): PositionedTextItem | null {
    ...
    const measuredHeight = Math.hypot(toFiniteNumber(transform[2]), toFiniteNumber(transform[3]));
    const height = Math.max(1, toFiniteNumber(item.height, measuredHeight || 10), measuredHeight);
    const estimatedWidth = Math.max(text.length * height * 0.45, height * 0.5);
    const width = Math.max(0, toFiniteNumber(item.width, estimatedWidth));
    return {
        text,
        x,
        y,
...
function joinItemsAsText(items: PositionedTextItem[]) {
    ...
    for (const item of sorted) {
        ...
        const characterWidth = Math.max(2, (getAverageCharacterWidth(previousItem) +
            getAverageCharacterWidth(item)) / 2);
        const shouldInsertSpace = gap > characterWidth * 0.25;
        const spaceCount = shouldInsertSpace
            ? Math.min(8, Math.max(1, Math.round(gap / characterWidth)))
            : 0;
...
function createExtractedCell(items: PositionedTextItem[]): ExtractedPdfCell | null {
...
function splitItemsIntoPositionedCells(items: PositionedTextItem[]) {
    const sorted = [...items].sort((a, b) => a.x - b.x);
    if (sorted.length === 0) {
        return [];
    }
    ...
    for (let index = 1; index < sorted.length; index += 1) {
        ...
        const characterWidth = Math.max(2, (getAverageCharacterWidth(previousItem) +
            getAverageCharacterWidth(item)) / 2);
        const cellBreakThreshold = Math.max(previousItem.height * 1.8, item.height * 1.8, characterWidth * 4.5, 18);
        if (gap > cellBreakThreshold ||
            previousItem.hasEOL) {
...
function groupItemsIntoLines(items: PositionedTextItem[]): ExtractedPdfLine[] {
    if (items.length === 0) {
        return [];
    }
    const medianHeight = getMedian(items.map((item) => item.height));
    const lineTolerance = Math.max(2, medianHeight * 0.45);
    ...
    for (const item of sorted) {
        ...
        currentLine.baseline =
            currentLine.items.reduce((total, lineItem) => total + lineItem.y, 0) / currentLine.items.length;
    }
    return lineGroups
        .map((group) => {
        const text = joinItemsAsText(group.items);
        const positionedCells = splitItemsIntoPositionedCells(group.items);
...
export async function extractTextFromPdf(selected: SelectedPdf | undefined, onProgress: ProgressCallback): Promise<ExtractedPdfDocument> {
    if (!selected) {
    ...
    try {
        ...
        for (let pageNumber = 1; pageNumber <= pdfDocument.numPages; pageNumber += 1) {
            onProgress(`A extrair texto da página ${pageNumber} de ${pdfDocument.numPages}...`);
        ...
        if (characterCount === 0) {
            throw new Error('Este PDF não contém texto selecionável. Se o documento for uma digitalização ou fotografia, será necessário OCR, que ainda não está disponível nesta ferramenta.');
        }
        return {
            pages,
