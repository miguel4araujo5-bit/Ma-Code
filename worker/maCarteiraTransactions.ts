...
export const MA_CARTEIRA_TRANSACTIONS_PATH =
...
export class MaCarteiraTransactionsError
  extends Error {
  status: number
...
const getDirection = (
  ...
  if (
    normalizedFrom === wallet &&
    normalizedTo === wallet
  ) {
    return 'self'
  }

  return normalizedFrom === wallet
    ? 'out'
    : 'in'
}
...
const supportsAccountApi = (
  family: ExplorerApiFamily
) =>
  family ===
    'blockscout-legacy' ||
  family ===
...
const finalizeTransactions = (
  ...
  const withoutDuplicateContracts =
    transactions.filter(
      (transaction) =>
        transaction.kind !==
          'contract' ||
  ...
  const directionsByHash =
    new Map<
      string,
      Set<TransactionDirection>
    >()

  withoutDuplicateContracts.forEach(
    (transaction) => {
      if (
        transaction.kind ===
        'contract'
  ...
  return withoutDuplicateContracts
    .map(
      (transaction) => {
        if (
          transaction.kind ===
